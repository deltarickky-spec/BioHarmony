...
export const INDIVIDUAL_SCANS = PLANS.filter((p) => p.kind === "individual");
export const PACKAGE_PLANS = PLANS.filter((p) => p.kind === "package");
export const PET_PLANS = PLANS.filter((p) => p.kind === "pet");
...
const AUDIO_PLAN_IDS = new Set([
  "ao-mindsync", "inner-voice", "emotional-balance",
